...
#Décomposition_martice_LU:-----------------------------------------------------------------

def decomp_LU(A):
    U=A.copy();L=np.identity(len(A))                            #U doir etre triangulaire superieur ; L doit etre triangulaire inferieur avec des 1 dans la diagonal
...
def LU_solve(A,b):
    ...
    for i in range(len(A)):
        Z[i]=b[i]-L[i,:i]@Z[:i]                     #Pas besoin de divisé par L[i,i] car est égale a 1
    X=np.zeros(len(A))
...
def LU_inv(A):
    ...
    for i in range(len(A)):
        ...
        Ap[:,i]=LU_solve(A,I[:,i])                      #On resout AX=b ( avec b est a chaque iteration une colone de l'identité et X est une colone de l'inverse de A)
...
def Choleski_decomp(A):                                 #La matrice A doit etre definie positif pour etre Choleski decomposable        
    ...
    return L                                            #on a donc A=L@transposé(L)
...
def Choleski_solve(A,b):
    ...
    X=np.zeros(len(A))
    for i in range(len(A)-1,-1,-1):
        X[i]=(Z[i]-L.T[i,i:]@X[i:])/L.T[i,i]            #Apres on resout (L.T)X=Z (avec L.T est la transposé de L)
...
def Gauss_inv(B):
    ...
    for i in range(len(A)-1,-1,-1):
        ...
        for j in range(i-1,-1,-1):
            ...
            coef=pivot/A[j,i]
            A[j,:]=A[j,:]*coef - A[i,:]                         #Ici la matrice est triangulaire inferieur donc diagonale
            Ap[j,:]=Ap[j,:]*coef - Ap[i,:]                          

        A[i,:]=A[i,:]/pivot                                     #On divise par le pivot pour avoir A une matrice identitée
        Ap[i,:]=Ap[i,:]/pivot
