...
from minimisation_1D import Dichotomous
from matrix_decomposition_inverse import Choleski_inv
from step_search import Goldstein
...
def Methode_Newton(f,x0,eps):
    ...
    while N_d>eps:
        ...
        alpha=Dichotomous(phi,0,10,eps)
        x1=x2
        x2=x1+alpha*d
...
def descant_gradient(f,x0,eps):
    ...
    while (N_f>eps or N_x>eps):
        ...
        N_f=np.linalg.norm(grad_f(x1))
        phi=lambda alpha : f(x1-alpha*grad_f(x1))
        alpha=Dichotomous(phi,0,10,eps)
...
def Conjugate_gradient(f,Q,x0,eps):
    ...
    while N_d>eps:
        ...
        x1=x2
        x2=x1+alpha*d
        beta=-gradient_f(x1).T@d/(d.T@Q@d)
        d=-gradient_f(x2)+beta*d
        N_d=np.linalg.norm(d)
